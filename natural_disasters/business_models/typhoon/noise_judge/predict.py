...
class Predictor(PredictorBase):
    ...
    def load_graph(self):
        ...
        self.model.saver.restore(self.sess,os.path.join(os.path.dirname(__file__),self.config["ckpt_model_path"],'textcnn-300'))
        # print('sucess')
        # 修改
        #ckpt = tf.train.get_checkpoint_state(os.path.join(os.path.abspath(os.path.dirname(__file__),self.config["ckpt_model_path"])))

        # if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        #     print(os.path.join(os.path.abspath(os.path.dirname(os.getcwd())),self.config["ckpt_model_path"]))
